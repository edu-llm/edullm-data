...
class Bundle:
    ...
    val_fraction: float = VAL_FRACTION
    #: Which slice of the SOURCE FILES this bundle reads, and how many slices the plan cut.
    #:
    #: ``file_shards == 1`` (the default) is the whole source, i.e. every behaviour that predates
    #: file-sharding, unchanged. ``file_shards == K > 1`` means the plan split one logical stream
    #: into K bundles that each read ``hf_files(spec)[file_shard::K]`` — a DISJOINT slice, striding
    #: rather than a contiguous block (see `_reader_for` and `_shard_slice`).
    #:
    #: **These come from the PLAN, never from `--shard/--of`.** `--shard/--of` strides the bundle
    ...
    def __post_init__(self) -> None:
        ...
        if self.file_shards < 1:
            raise BuildDriverError(
                f"{self.bundle_id}: file_shards={self.file_shards} — must be >= 1 (1 = the whole "
                f"source, the pre-file-sharding behaviour)"
            )
        if not 0 <= self.file_shard < self.file_shards:
            raise BuildDriverError(
                f"{self.bundle_id}: file_shard={self.file_shard} is out of range for "
                f"file_shards={self.file_shards}. Out-of-range is not a no-op: "
                f"`items[{self.file_shard}::{self.file_shards}]` returns [] for a shard past the "
                f"end, and a child that reads NO files still writes a receipt, so the bundle would "
                f"be silently empty rather than failed."
            )
    ...
    def from_plan_entry(cls, entry: Mapping[str, Any],
        ...
        return cls(
            ...
            file_shard=int(entry.get("file_shard", 0)),
            file_shards=int(entry.get("file_shards", 1)),
        )
...
def _assert_file_shard_family(bundles: Sequence[Bundle]) -> None:
    ...
    **This is what makes `_reader_for`'s decision not to divide the budget by K safe.** That
    ...
    for (src, dom, split, _key), sibs in sorted(
        ...
        ks = {b.file_shards for b in sibs}
        if len(ks) != 1:
            raise BuildDriverError(
        ...
        k = ks.pop()
        got = sorted(b.file_shard for b in sibs)
        if got != list(range(k)):
            raise BuildDriverError(
...
def _bundle_files(spec: CorpusSpec, bundle: Bundle) -> list[dict]:
    ...
    """
    files = hf_files(spec)
    if bundle.file_shards == 1:
        return files
    if bundle.file_shards > len(files):
        raise BuildDriverError(
            f"{bundle.bundle_id}: the plan splits this source {bundle.file_shards} ways but "
            ...
            f"Lower file_shards to at most {len(files)}."
        )
    return _shard_slice(files, bundle.file_shard, bundle.file_shards)
...
def _reader_for(spec: CorpusSpec, bundle: Bundle) -> Iterable[Document]:
    ...
    **Feasibility survives the split exactly, and only because the stride is even.** The child gets
    ...
    # NOT divided by `bundle.file_shards` — the docstring's "THE BUDGET UNDER FILE-SHARDING"
